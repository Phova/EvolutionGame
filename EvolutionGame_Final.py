...
class Player:
    ...
    def get_hand_limit(self) -> int:
        ...
        evolution_cards_count = len(self.hand[CardType.EVOLUTION])
        # 修改为：进化卡数量 + 1
        base_limit = evolution_cards_count + 1
        return max(0, base_limit + self.hand_limit_bonus)
...
class EvolutionGame:
    ...
    def replenish_strategy_cards(self, players: List[Player], card_type: CardType = None, specific_type: ActionType = None):
        ...
        for player in players:
            ...
            # 检查手牌上限
            current_strategy_cards = len(player.hand[CardType.COOPERATION]) + len(player.hand[CardType.DECEPTION])
            if current_strategy_cards >= player.get_hand_limit():
                continue  # 手牌已达上限，不补充
    ...
    def resolve_trust_evolution(self, current_player: Player):
        ...
        for player in self.players:
            ...
            # 使用一张牌并回收至牌库
            self.recycle_strategy_card(player, actions[player])
    ...
    """def resolve_trust_evolution(self, current_player: Player):
        ...
        # 首先处理相互指定的玩家对（双向关系）
        for player1, target1 in targets.items():
            if player1.is_eliminated or player1 not in actions:
                continue
                
            # 检查是否为相互指定
            if target1 in targets and targets[target1] == player1:
                pair = tuple(sorted([player1.name, target1.name]))
                ...
                # 结算这对玩家的互动
                self._resolve_trust_pair(player1, target1, actions[player1], actions[target1])
        
        # 然后处理单向指定的玩家（没有形成相互关系的玩家）
        for player, target in targets.items():
            if player.is_eliminated or player not in actions:
                continue
                
            # 检查是否已经处理过（作为相互对的一部分）
            pair_with_target = tuple(sorted([player.name, target.name]))
            if pair_with_target in processed_pairs:
                continue
                
            # 处理单向关系：只结算指定方的行动
            self._resolve_trust_single(player, target, actions[player])
        
        # 所有玩家可补充一张合作或欺骗卡
        self.replenish_strategy_cards([p for p in self.players if not p.is_eliminated])"""

    def _resolve_trust_pair(self, player1, player2, action1, action2):
        """处理相互指定的玩家对"""
        # 共生纽带效果检查
        symbiosis_bonus_player1 = (action1 == ActionType.COOPERATE and player1.symbiosis_target == player2)
        symbiosis_bonus_player2 = (action2 == ActionType.COOPERATE and player2.symbiosis_target == player1)
        
        if action1 == ActionType.COOPERATE and action2 == ActionType.COOPERATE:
            # 合作 vs 合作：双方各获得2点进化点数
            player1.evolution_points += 2
            player2.evolution_points += 2
            self.log(f"{player1.name} 和 {player2.name} 相互合作，各获得2点进化点数")
            
            # 共生纽带效果
            if symbiosis_bonus_player1:
                evolution_card = self.evolution_deck.draw(1)
                if evolution_card:
                    player1.add_card(evolution_card[0], self.log)
                    self.log(f"{player1.name} 的共生纽带效果触发，额外获得1张进化卡")
            if symbiosis_bonus_player2:
                evolution_card = self.evolution_deck.draw(1)
                if evolution_card:
                    player2.add_card(evolution_card[0], self.log)
                    self.log(f"{player2.name} 的共生纽带效果触发，额外获得1张进化卡")
            
        elif action1 == ActionType.COOPERATE and action2 == ActionType.DECEIVE:
            # 合作 vs 欺骗：出合作方获得1点进化点数，出欺骗方抽取3张进化卡
            player1.evolution_points += 1
            evolution_cards = self.evolution_deck.draw(3)
            for card in evolution_cards:
                player2.add_card(card, self.log)
            self.log(f"{player1.name} 被 {player2.name} 欺骗，{player1.name} 获得1点进化点数，{player2.name} 抽取3张进化卡")
            
        elif action1 == ActionType.DECEIVE and action2 == ActionType.COOPERATE:
            # 欺骗 vs 合作：出欺骗方抽取3张进化卡，出合作方获得1点进化点数
            evolution_cards = self.evolution_deck.draw(3)
            for card in evolution_cards:
                player1.add_card(card, self.log)
            player2.evolution_points += 1
            self.log(f"{player1.name} 欺骗了 {player2.name}，{player1.name} 抽取3张进化卡，{player2.name} 获得1点进化点数")
            
        else:  # 欺骗 vs 欺骗
            # 双方各弃置1张进化卡
            if player1.has_evolution_cards(1):
                # 顽强生命力效果
                if player1.has_ability("顽强生命力"):
                    self.log(f"{player1.name} 的顽强生命力效果触发，无需弃置进化卡")
                else:
                    player1.remove_card(CardType.EVOLUTION, 1, self.log)
            
            if player2.has_evolution_cards(1):
                # 顽强生命力效果
                if player2.has_ability("顽强生命力"):
                    self.log(f"{player2.name} 的顽强生命力效果触发，无需弃置进化卡")
                else:
                    player2.remove_card(CardType.EVOLUTION, 1, self.log)
            
            self.log(f"{player1.name} 和 {player2.name} 互相欺骗，各弃置1张进化卡")
        
        # 应用能力效果
        self.apply_ability_effects(player1, action1, "trust_evolution", player2)
        self.apply_ability_effects(player2, action2, "trust_evolution", player1)

    def _resolve_trust_single(self, player, target, action):
        """处理单向指定的玩家（没有形成相互关系的玩家）"""
        # 单向指定时，只结算指定方的行动，被指定方不参与结算（除非他们也指定了该玩家）
        if action == ActionType.COOPERATE:
            # 单向合作：无效果（因为没有相互指定）
            self.log(f"{player.name} 单向合作指定 {target.name}，但未形成相互指定，无效果")
        else:  # ActionType.DECEIVE
            # 单向欺骗：无效果（因为没有相互指定）
            self.log(f"{player.name} 单向欺骗指定 {target.name}，但未形成相互指定，无效果")
        
        # 单向关系下，仍然可以触发某些能力效果
        self.apply_ability_effects(player, action, "trust_evolution", target)
    ...
    def resolve_hawk_dove(self, current_player: Player):
        ...
        # 使用一张牌并回收至牌库
        self.recycle_strategy_card(current_player, current_action)
        self.recycle_strategy_card(opponent, opponent_action)
    ...
    def resolve_resource_rich(self, current_player: Player):
        ...
        if climate_change and not hasattr(current_player, '_climate_change_processed'):
            self.log("气候剧变效果生效，资源丰富格被视为自然灾害格")
            # 设置标记避免递归
            current_player._climate_change_processed = True
            try:
                self.resolve_natural_disaster(current_player)
            finally:
                # 确保标记被清除
                delattr(current_player, '_climate_change_processed')
            return
        ...
        for player in self.players:
            ...
            # 使用一张牌
            if action == ActionType.COOPERATE:
                if self.recycle_strategy_card(player, action):  # 先回收卡牌
                    # 出合作卡：抽取2张进化卡
                    cards_to_draw = 2
    ...
    """def resolve_cooperation_sanctuary(self, current_player: Player):
        ...
        for player in self.players:
            ...
            # 使用一张牌并回收至牌库
            if action == ActionType.COOPERATE:
                self.recycle_strategy_card(player, action)
            else:
                self.recycle_strategy_card(player, action)
    ...
    def resolve_cooperation_sanctuary(self, current_player: Player):
        ...
        # 记录所有玩家的选择和状态
        actions = {}
        cooperation_players = []  # 出合作卡的玩家列表
        deception_players = []    # 出欺骗卡的玩家列表
        chosen_players = set()    # 已经被选择过的玩家
        ...
        # 第二步：结算合作卡效果 - 每个合作玩家可以选择另一个合作玩家
        # 随机打乱合作玩家列表，确保公平性
        random.shuffle(cooperation_players)
        
        for player in cooperation_players:
            # 如果玩家已经被选择过，跳过
            if player in chosen_players:
                continue
                
            # 寻找其他可选择的合作玩家（未被选择过的）
            available_partners = [p for p in cooperation_players 
                                if p != player and p not in chosen_players]
            ...
            if available_partners:
                ...
                self.log(f"{player.name} 选择了与 {partner.name} 合作，双方各抽取3张进化卡")
                
                # 标记这两个玩家为已选择
                chosen_players.add(player)
                chosen_players.add(partner)
                ...
                # 没有其他合作玩家可选择，无收益
                self.log(f"{player.name} 选择合作，但没有其他合作者可以选择，无收益")
                chosen_players.add(player)  # 标记为已处理
        
        # 第三步：结算欺骗卡效果
        for player in deception_players:
            # 单独抽取4张进化卡
            evolution_cards = self.evolution_deck.draw(4)
            for card in evolution_cards:
                player.add_card(card, self.log)
            ...
            next_player_index = (self.players.index(player) + 1) % len(self.players)
            next_player = self.players[next_player_index]
            if not next_player.is_eliminated and next_player != player:
                next_player.free_challenge_target = player
                self.log(f"{next_player.name} 获得对 {player.name} 的免费挑战权")   
    ...
    def ecological_investigation(self, current_player: Player):
        ...
        # 检查手牌是否超过上限（考虑新手牌上限）
        if total_strategy_cards > current_player.get_hand_limit():
            self.log(f"{current_player.name} 手牌超过上限，需要先弃牌")
        current_player.check_hand_limit(self.log)
    ...
    def play_game(self):
        ...
        round_count = 1
        while not self.game_over and round_count <= 10000:  # 防止无限循环
            self.log(f"\n--- 第 {round_count} 轮 ---")
            self.play_turn()
            round_count += 1
    ...
    def recycle_strategy_card(self, player: Player, action_type: ActionType):
        """将打出的策略卡回收至对应牌库（摸牌堆）"""
        if action_type == ActionType.COOPERATE and player.hand[CardType.COOPERATION]:
            card = player.hand[CardType.COOPERATION].pop()
            # 将卡牌加入牌库（随机位置）
            insert_index = random.randint(0, len(self.cooperation_deck.cards))
            self.cooperation_deck.cards.insert(insert_index, card)
            self.log(f"{player.name} 打出合作卡并回收至牌库")
            return True
        elif action_type == ActionType.DECEIVE and player.hand[CardType.DECEPTION]:
            card = player.hand[CardType.DECEPTION].pop()
            # 将卡牌加入牌库（随机位置）
            insert_index = random.randint(0, len(self.deception_deck.cards))
            self.deception_deck.cards.insert(insert_index, card)
            self.log(f"{player.name} 打出欺骗卡并回收至牌库")
            return True
        return False
        ...
        while True:
            ...
            if name:
                if name not in player_names:
                    ...
                    print("玩家名称不能重复，请重新输入。")
            else:
                print("玩家名称不能为空，请重新输入。")
