...
class GameBoard:
    ...
    def display_map(self, players: List[Player]):
        """显示地图和玩家位置"""
        print("\n" + "="*60)
        print("游戏地图")
        print("="*60)
        ...
        # 显示格子类型说明
        print("\n格子类型说明:")
        print("起:起点格 | 信:信任进化格 | 鹰:鹰鸽博弈格 | 资:资源丰富格 | 灾:自然灾害格")
        print("合:合作圣地格 | 欺:欺骗沼泽格 | 事:突变事件格 | 实:进化实验室格 | 终:终点格")
        print("="*60)
...
class EvolutionGame:
    def __init__(self, player_names: List[str]):
        ...
        # 游戏开始时显示地图
        self.board.display_map(self.players)
    ...
    def next_player(self):
        ...
        # 跳过已被淘汰的玩家
        while self.get_current_player().is_eliminated:
            self.current_player_index = (self.current_player_index + (1 if self.turn_order_clockwise else -1)) % len(self.players)
    ...
    def resolve_trust_evolution(self, current_player: Player):
        """解析信任进化格效果"""
        print(f"{current_player.name} 触发了信任进化格")
        ...
        for player, target in targets.items():
            ...
            if player_action == ActionType.COOPERATE and target_action == ActionType.COOPERATE:
                ...
                print(f"{player.name} 和 {target.name} 合作，各获得2点进化点数")
                ...
                print(f"{player.name} 被 {target.name} 欺骗，{player.name} 获得1点进化点数，{target.name} 抽取3张进化卡")
                ...
                print(f"{player.name} 欺骗了 {target.name}，{player.name} 抽取3张进化卡，{target.name} 获得1点进化点数")
            else:  # 欺骗 vs 欺骗
                # 双方各弃置1张进化卡
                ...
                print(f"{player.name} 和 {target.name} 互相欺骗，各弃置1张进化卡")
        ...
        for player in self.players:
            ...
            # 简化实现：随机补充
            if random.random() < 0.5:
                card = self.cooperation_deck.draw(1)[0]
            else:
                card = self.deception_deck.draw(1)[0]
            player.add_card(card)
    
    def resolve_hawk_dove(self, current_player: Player):
        """解析鹰鸽博弈格效果"""
        print(f"{current_player.name} 触发了鹰鸽博弈格")
        ...
        print(f"{current_player.name} 选择了与 {opponent.name} 进行鹰鸽博弈")
        ...
        if current_action == ActionType.COOPERATE and opponent_action == ActionType.COOPERATE:
            ...
            print(f"{current_player.name} 和 {opponent.name} 都选择合作，各获得2点进化点数")
            ...
            print(f"{current_player.name} 欺骗了 {opponent.name}，{current_player.name} 抽取3张进化卡，{opponent.name} 获得1点进化点数")
            ...
            print(f"{opponent.name} 欺骗了 {current_player.name}，{opponent.name} 抽取3张进化卡，{current_player.name} 获得1点进化点数")
        else:  # 鹰 vs 鹰
            # 双方各弃置2张进化卡
            ...
            print(f"{current_player.name} 和 {opponent.name} 都选择欺骗，各弃置2张进化卡")
    ...
    def resolve_resource_rich(self, current_player: Player):
        """解析资源丰富格效果"""
        print(f"{current_player.name} 触发了资源丰富格")
        ...
        for player in self.players:
            ...
            if action == ActionType.COOPERATE:
                ...
                for card in evolution_cards:
                    player.add_card(card)
                print(f"{player.name} 选择合作，抽取2张进化卡")
            else:
                player.remove_card(CardType.DECEPTION)
                ...
                if dice_roll >= 5:
                    ...
                    for card in evolution_cards:
                        player.add_card(card)
                    print(f"{player.name} 选择欺骗，掷出{dice_roll}，抽取4张进化卡")
                else:
                    print(f"{player.name} 选择欺骗，掷出{dice_roll}，无所获")
    
    def resolve_natural_disaster(self, current_player: Player):
        """解析自然灾害格效果"""
        print(f"{current_player.name} 触发了自然灾害格")
        ...
        for player in self.players:
            ...
            if action == ActionType.COOPERATE:
                player.remove_card(CardType.COOPERATION)
                # 出合作卡：免疫本次灾害
                print(f"{player.name} 选择合作，免疫自然灾害")
            else:
                player.remove_card(CardType.DECEPTION)
                ...
                if player.has_evolution_cards(1):
                    ...
                    if available_targets:
                        ...
                        if target.has_evolution_cards(1):
                            target.remove_card(CardType.EVOLUTION)
                            print(f"{player.name} 选择欺骗，弃置1张进化卡，并让 {target.name} 也弃置1张进化卡")
                        else:
                            print(f"{player.name} 选择欺骗，弃置1张进化卡，但 {target.name} 没有进化卡可弃置")
                    else:
                        print(f"{player.name} 选择欺骗，弃置1张进化卡，但没有其他玩家可指定")
                else:
                    print(f"{player.name} 选择欺骗，但没有进化卡可弃置")
    
    def resolve_cooperation_sanctuary(self, current_player: Player):
        """解析合作圣地格效果"""
        print(f"{current_player.name} 触发了合作圣地格")
        ...
        for player in cooperators:
            ...
            if other_cooperators:
                ...
                print(f"{player.name} 和 {partner.name} 合作，各抽取3张进化卡")
            else:
                print(f"{player.name} 选择合作，但没有其他合作者，无收益")
        ...
        for player in deceivers:
            ...
            for card in evolution_cards:
                player.add_card(card)
            print(f"{player.name} 选择欺骗，单独抽取4张进化卡")
            ...
            if not next_player.is_eliminated:
                # 记录挑战权（简化实现）
                print(f"{next_player.name} 获得对 {player.name} 的免费挑战权")
    
    def resolve_deception_swamp(self, current_player: Player):
        """解析欺骗沼泽格效果"""
        print(f"{current_player.name} 触发了欺骗沼泽格")
        ...
        for player, action in actions.items():
            if action == ActionType.DECEIVE:
                ...
                print(f"{player.name} 选择欺骗，查看能力牌库顶3张牌并拿取1张")
                # 简化实现：随机获得一张能力卡
                if self.ability_deck.cards:
                    ability_card = self.ability_deck.draw(1)[0]
                    player.add_card(ability_card)
                    print(f"{player.name} 获得了能力卡: {ability_card.name}")
                ...
                if deceivers:
                    ...
                    # 阻止目标获得能力卡（简化实现）
                    print(f"{player.name} 选择合作，阻止了 {target.name} 获得能力卡，并抽取1张进化卡")
                    evolution_card = self.evolution_deck.draw(1)[0]
                    player.add_card(evolution_card)
                else:
                    print(f"{player.name} 选择合作，但没有欺骗者可指定，无收益")
    
    def resolve_mutation_event(self, current_player: Player):
        """解析突变事件格效果"""
        print(f"{current_player.name} 触发了突变事件格")
        
        # 抽一张事件卡并立即执行其效果
        if self.event_deck.cards:
            event_card = self.event_deck.draw(1)[0]
            print(f"{current_player.name} 抽到事件卡: {event_card.name}")
            # 简化实现：随机效果
            ...
            if effect_type == 1:
                ...
                for card in evolution_cards:
                    current_player.add_card(card)
                # 记录下回合无法移动（简化实现）
                print(f"{current_player.name} 获得基因突变效果：抽取3张进化卡，但下回合无法移动")
            elif effect_type == 2:
                ...
                for player in self.players:
                    ...
                    print(f"{player.name} 移动到位置 {next_position}")
            elif effect_type == 3:
                ...
                if available_players:
                    ...
                    for card in evolution_cards:
                        target.add_card(card)
                    print(f"{current_player.name} 和 {target.name} 互利共生，各抽取2张进化卡")
            elif effect_type == 4:
                # 自然选择：当前进化卡数量最少的玩家必须弃置2张进化卡
                min_evolution_count = min(p.get_evolution_card_count() for p in self.players if not p.is_eliminated)
                victims = [p for p in self.players if not p.is_eliminated and p.get_evolution_card_count() == min_evolution_count]
                for victim in victims:
                    if victim.has_evolution_cards(2):
                        victim.remove_card(CardType.EVOLUTION, 2)
                        print(f"{victim.name} 被自然选择，弃置2张进化卡")
                    elif victim.has_evolution_cards(1):
                        victim.remove_card(CardType.EVOLUTION, 1)
                        print(f"{victim.name} 被自然选择，弃置1张进化卡")
                    else:
                        print(f"{victim.name} 被自然选择，但没有进化卡可弃置")
            else:
                ...
                print(f"{current_player.name} 获得加速进化效果：抽取2张进化卡，并移动到位置 {current_player.position} ({new_grid.grid_type.value})")
    ...
    def resolve_evolution_lab(self, current_player: Player):
        """解析进化实验室格效果"""
        print(f"{current_player.name} 触发了进化实验室格")
        ...
        if current_player.has_evolution_cards(2):
            ...
            if self.ability_deck.cards:
                ...
                print(f"{current_player.name} 支付2张进化卡，获得了能力卡: {ability_card.name}")
            else:
                print("能力牌库已空，无效果")
        else:
            print(f"{current_player.name} 没有足够的进化卡支付代价")
    
    def resolve_finish(self, current_player: Player):
        """解析终点格效果"""
        print(f"{current_player.name} 抵达了终点格")
        ...
        for card in evolution_cards:
            current_player.add_card(card)
        current_player.evolution_points += 2
        print(f"{current_player.name} 获得3张进化卡和2点进化点数")
    ...
    def optional_actions(self, current_player: Player):
        ...
        if random.random() < 0.5:
            ...
            if current_player.evolution_points > 0:
                ...
                print(f"{current_player.name} 兑换了{points_to_convert}点进化点数，获得{points_to_convert}张进化卡")
    ...
    def environment_change(self, current_player: Player):
        """执行环境变化（可选规则）"""
        # 简化实现：随机决定是否执行环境变化
        if random.random() < 0.3 and self.environment_change_deck.cards:
            environment_card = self.environment_change_deck.draw(1)[0]
            print(f"{current_player.name} 抽到环境变化卡: {environment_card.name}")
    ...
    def display_player_status(self):
        ...
        for player in self.players:
            if player.is_eliminated:
                print(f"{player.name} ({player.marker}): 已淘汰")
            else:
                print(f"{player.name} ({player.marker}): 位置{player.position}, 进化卡{player.get_evolution_card_count()}张, 进化点数{player.evolution_points}点")
    ...
    def play_turn(self):
        ...
        # 阶段一：掷骰与移动
        dice_roll = self.roll_dice()
        print(f"{current_player.name} 掷出 {dice_roll} 点")
        
        self.move_player(current_player, dice_roll)
        new_grid = self.board.get_grid(current_player.position)
        print(f"{current_player.name} 移动到位置 {current_player.position} ({new_grid.grid_type.value})")
